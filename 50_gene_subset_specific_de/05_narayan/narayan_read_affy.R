...
if(!file.exists("./working_data/subset/narayan_raw_eset.rds")) {
  #READ AFFY####
  affy.folder <- "./raw_data/datasets/Narayan_GSE21138/"
  ...
  meta <- read.csv("./05_subset_analysis/05_narayan/Narayan_GSE21138_datMeta.csv")
  ...
  plot(mds, col=as.numeric(as.factor(meta$duration)), pch=16, main="MDS Plot")
  legend("topright", levels(as.factor(meta$duration)), col=seq(length(levels(as.factor(meta$duration)))), pch=16, cex=0.8)
  ...
  saveRDS(eset, "./working_data/subset/narayan_raw_eset.rds")
}
#QC, correlation####

if(!file.exists("./working_data/subset/narayan_regressed_combat.rds")) {
  eset <- readRDS("./working_data/subset/narayan_raw_eset.rds")
  meta <-
    pData(eset)[, c("disease.state",
                    "sex",
                    "age",
                    "pmi",
                    "ph",
  ...
  eset_exprs <- exprs(eset)
  X <- model.matrix( ~ disease.state + sex + age + ph + pmi + rna.deg, data =
                       meta)
  Y <- eset_exprs
  beta = (solve(t(X) %*% X) %*% t(X)) %*% t(Y)
  
  reg_corr <- (as.matrix(X[, 4:7]) %*% (as.matrix(beta[4:7, ])))
  eset_exprs <- eset_exprs - t(reg_corr)
  eset <- ExpressionSet(eset_exprs)
  pData(eset) <- meta
  
  saveRDS(eset, "./working_data/subset/narayan_regressed_combat.rds")
}
1
if(F){
eset <- readRDS("./working_data/subset/narayan_regressed_combat.rds")
meta <- pData(eset)
...
plot(mds$points, col=as.numeric(as.factor(meta$duration)), pch=16, main="MDS Plot", asp=1, 
     xlab = paste("PC1 (", signif(100*pc1,3), "%)", sep=""), ylab = paste("PC1 (", signif(100*pc2,3),"%)",sep=""))
legend("bottomright", levels(as.factor(meta$duration)), col=c(1:length(levels(as.factor(meta$duration)))), pch=16, cex=0.8)
...
#save####
saveRDS(DGE, file = "./working_data/subset/dge/narayan_chol_scz.rds")



#SEX DIFF####
meta$dis_sex <- factor(paste(meta$Group, meta$Sex, sep = "_"))
...
saveRDS(DGE, file = "./working_data/subset/dge/narayan_chol_scz_sex.rds")
...
#save####
saveRDS(DGE, file = "./working_data/subset/dge/narayan_mtar_scz.rds")
...
#save####
saveRDS(DGE, file = "./working_data/subset/dge/narayan_mtar_scz_sex.rds")
